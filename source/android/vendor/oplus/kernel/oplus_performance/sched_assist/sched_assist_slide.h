...
{
    	return sched_assist_scene(SA_SLIDE) && is_heavy_ux_task(p) && _slide_task_misfit(p, cpu);
}

static void inline slide_set_start_cpu(struct root_domain *rd, struct task_struct *p, int *start_cpu)
{
    if ((sched_assist_scene(SA_SLIDE) || sysctl_animation_type == LAUNCHER_SI_START) && is_heavy_ux_task(p) ) {
