...
u64 _slide_get_boost_load(int cpu) {
    ...
    if ((sched_assist_scene(SA_SLIDE) || sysctl_animation_type == LAUNCHER_SI_START) && ux_task_load[cpu]) {
...
void slide_calc_boost_load(struct rq *rq, unsigned int *flag, int cpu) {
	...
	if (sched_assist_scene(SA_SLIDE) || sysctl_animation_type == LAUNCHER_SI_START) {
